...
atomic_valid_bond[(1, 6)] = 1.19
atomic_valid_bond[(1, 7)] = 1.11
atomic_valid_bond[(1, 8)] = 1.06
atomic_valid_bond[(6, 6)] = 1.64
atomic_valid_bond[(6, 7)] = 1.57
atomic_valid_bond[(6, 8)] = 1.53
atomic_valid_bond[(6, 9)] = 1.45
atomic_valid_bond[(7, 7)] = 1.55
atomic_valid_bond[(7, 8)] = 1.50

def get_mol_flag(x, pos, adj):
    dMat = distance_matrix(pos, pos)  # pos : [N, N]
    n_atoms = np.argmin(x)
    is_valid = 1
    for i in range(1, n_atoms):
        for j in range(0, i):
            if adj[i][j] > 0:
                if (min(x[i], x[j]), max(x[i], x[j]), adj[i][j]) in atomic_valid_bond:
                    threshold = atomic_valid_bond[(min(x[i], x[j]), max(x[i], x[j]), adj[i][j])]
