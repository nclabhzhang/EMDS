...
class SDE(abc.ABC):
  ...
  def reverse(self, score_fn, probability_flow=False):
    ...
    class RSDE(self.__class__):
      ...
      def sde(self, x, pos, adj, flags, t, type):
        """Create the drift and diffusion functions for the reverse SDE/ODE."""
        if type == 'x':
          drift, diffusion = sde_fn(x, t)
        elif type == 'pos':
          drift, diffusion = sde_fn(pos, t)
        else:
          drift, diffusion = sde_fn(adj, t)
        score = score_fn(x, pos, adj, flags, t)
        drift = drift - diffusion[:, None, None] ** 2 * score * (0.5 if self.probability_flow else 1.)
        # -------- Set the diffusion function to zero for ODEs. --------
        diffusion = 0. if self.probability_flow else diffusion
        return drift, diffusion

      def discretize(self, x, pos, adj, flags, t, type):    # feature = node(x), while x = adj matrix
        """Create discretized iteration rules for the reverse diffusion sampler."""
        if type == 'x':
          f, G = discretize_fn(x, t)
        elif type == 'pos':
          f, G = discretize_fn(pos, t)
        else:
          f, G = discretize_fn(adj, t)
        score = score_fn(x, pos, adj, flags, t)
...
class VESDE(SDE):
  ...
  def prior_sampling(self, shape):  # Generate one sample from the prior distribution, p_T(x), p_T(P)
    return torch.randn(*shape) * self.sigma_max

  def prior_sampling_sym(self, shape):  # Generate one sample from the prior distribution, p_T(A)
    x = torch.randn(*shape).triu(1) * self.sigma_max
    x = x + x.transpose(-1, -2)
    return x

  def prior_logp(self, z):  # Compute log-density of the prior distribution. Useful for computing the log-likelihood via probability flow ODE.
    shape = z.shape
    N = np.prod(shape[1:])
    logps = -N / 2. * np.log(2 * np.pi * self.sigma_max ** 2) - torch.sum(z ** 2, dim=(1, 2, 3)) / (2 * self.sigma_max ** 2)
    return logps
  ...
  def discretize(self, x, t):
    ...
    G = torch.sqrt(sigma ** 2 - adjacent_sigma ** 2)   # sqrt(sigma_{i+1}^2 - sigma_{i}^2)
    return f, G
