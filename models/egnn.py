...
class EquivariantBlock(nn.Module):
    ...
    def forward(self, h, x, edge_index, node_mask=None, edge_mask=None, edge_attr=None):
        ...
        angles = angel_information(x, edge_attr.shape[0]//x.shape[0])
        distances = torch.cat([distances, angles], dim=1)
...
class EGNN(nn.Module):
    def __init__(self, in_node_nf, in_edge_nf, hidden_nf, act_fn=nn.SiLU(), n_layers=3, attention=False,
        ...
        if out_node_nf is None:  # None, so: out_node_nf = in_node_nf
...
def angel_information(pos, n_atom):
    pos = pos.view(-1, n_atom, 3)
