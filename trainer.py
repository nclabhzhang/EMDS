...
class Trainer(object):
    def __init__(self, config):
        ...
        self.params_x, self.params_pos, self.params_adj = load_model_params(self.config)
    ...
    def train(self, ts):
        ...
        self.model_pos, self.optimizer_pos, self.scheduler_pos = load_model_optimizer(self.params_pos, self.config.train, self.device)
        ...
        self.ema_pos = load_ema(self.model_pos, decay=self.config.train.ema)
        self.ema_adj = load_ema(self.model_adj, decay=self.config.train.ema)

        save_dir = f'./checkpoints/{self.config.data.data}/{self.ckpt}'   # save_dir for training checkpoints
        ...
        for epoch in trange(0, (self.config.train.num_epochs), desc='[Epoch]', position=1, leave=False):
            self.train_x, self.train_pos, self.train_adj = [], [], []
            self.test_x ,self.test_pos, self.test_adj = [], [], []
            t_start = time.time()
            ...
            for _, train_b in enumerate(self.train_loader):
                ...
                x, pos, adj = load_batch(train_b, self.device)
                loss_subject = (x, pos, adj)
                loss_x, loss_pos, loss_adj = self.loss_fn(self.model_x, self.model_pos, self.model_adj, *loss_subject)
                self.train_x.append(loss_x.item())
                self.train_pos.append(loss_pos.item())
                ...
                torch.nn.utils.clip_grad_norm_(self.model_pos.parameters(), self.config.train.grad_norm)
                ...
                self.ema_pos.update(self.model_pos.parameters())
                self.ema_adj.update(self.model_adj.parameters())

                sys.stdout.write('\r[Epoch %d / %d] Train | step: %d | loss_sum: %f | loss_x: %f, loss_pos: %f, loss_adj: %f' % (epoch+1,  self.config.train.num_epochs, _+1, loss_x+loss_pos+loss_adj, loss_x, loss_pos, loss_adj))
                sys.stdout.flush()
            ...
            for _, test_b in enumerate(self.test_loader):
                x, pos, adj = load_batch(test_b, self.device)
                loss_subject = (x, pos, adj)

                with torch.no_grad():
                    self.ema_x.store(self.model_x.parameters())
                    self.ema_x.copy_to(self.model_x.parameters())
                    self.ema_pos.store(self.model_pos.parameters())
                    self.ema_pos.copy_to(self.model_pos.parameters())
                    self.ema_adj.store(self.model_adj.parameters())
                    self.ema_adj.copy_to(self.model_adj.parameters())

                    loss_x, loss_pos, loss_adj = self.loss_fn(self.model_x, self.model_pos, self.model_adj, *loss_subject)
                    self.test_x.append(loss_x.item())
                    self.test_pos.append(loss_pos.item())
                    self.test_adj.append(loss_adj.item())

                    self.ema_x.restore(self.model_x.parameters())
                    self.ema_pos.restore(self.model_pos.parameters())
                    self.ema_adj.restore(self.model_adj.parameters())

                sys.stdout.write('\r[Epoch %d / %d] Test | step: %d | loss_sum: %f | loss_x: %f, loss_pos: %f, loss_adj: %f' % (epoch+1,  self.config.train.num_epochs, _+1, loss_x+loss_pos+loss_adj, loss_x, loss_pos, loss_adj))
                sys.stdout.flush()
            ...
            mean_test_pos = np.mean(self.test_pos)
            mean_test_adj = np.mean(self.test_adj)

            # -------- Log losses --------
            logger.log(f'{epoch+1:03d} | {time.time()-t_start:.2f}s | '
                       f'test x: {mean_test_x:.3e} | test pos: {mean_test_pos:.3e} | test adj: {mean_test_adj:.3e} | '
                       f'train x: {mean_train_x:.3e} | train pos: {mean_train_pos:.3e} |train adj: {mean_train_adj:.3e} | ', verbose=False)

            # -------- Save checkpoints --------
            if epoch % self.config.train.save_interval == self.config.train.save_interval - 1:
                save_dir = f'./checkpoints/{self.config.data.data}/{self.ckpt}'
                if not os.path.exists(save_dir):
                    os.makedirs(save_dir)
            ...
            if epoch % self.config.train.print_interval == self.config.train.print_interval-1:
                tqdm.write(f'[EPOCH {epoch+1:04d}] test adj: {mean_test_adj:.3e} | train adj: {mean_train_adj:.3e} | '
                                                 ...
                                                 f'test pos: {mean_test_pos:.3e} | train pos: {mean_train_pos:.3e}')
        print(' ')
        return self.ckpt
